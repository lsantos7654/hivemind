...
from hivemind_cli.core import _get_head_commit, _load_json, _save_json
...
class TestSaveJson:
    def test_atomic_write(self, tmp_path: Path):
        """Verify _save_json uses atomic write (temp + replace)."""
        p = tmp_path / "config.json"
        _save_json(p, {"test": True})
        assert p.exists()
